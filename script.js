...
function shop() {
  ...
  buytripleButton.addEventListener('click', () => {
    if (currentCash >= 2500000) {
      currentProtection += 3; // currentProtection 증가
      currentCash -= 2500000; // currentCash 차감
      update();
      // 결과를 사용자에게 표시
      alert(`구매 성공! 방지권 수치: ${currentProtection}, 잔액: ${currentCash}원`);
    } else {
